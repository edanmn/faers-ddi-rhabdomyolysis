...
\textbf{Key result.} Both statistics are log₂((n + α)/(E + α)) under identical
shrinkage, so E\_mult ≥ E\_add forces \{Ω₀₂₅ \textgreater{} 0\} ⊆ \{Ω\_add,₀₂₅ \textgreater{} 0\}: the nulls
become one ordering read at two thresholds. That ordering holds for
\textbf{98.3\%} of pairs whose expected joint rate
exceeds 5\% and only \textbf{16.9\%} of pairs below
0.5\%, with \textbf{no exception among 13,208 ordered pairs}. Both
...
\tightlist
\item
  \textbf{A pre-hoc test for whether the choice of null matters at all.} Both
  ...
  E\_mult ≥ E\_add forces the multiplicative signal set to lie \emph{inside} the
  additive one. We show that antecedent holds for \textbf{98.3\%} of pairs whose
  expected joint rate exceeds 5\% and only \textbf{16.9\%} of pairs below 0.5\%, with
  \textbf{no exception in 13,208 ordered pairs}. Where it holds, the two nulls
