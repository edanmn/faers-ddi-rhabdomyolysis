...
\title{A pre-hoc test for when disproportionality nulls collapse to one, with calibrated in-regime error rates for drug--drug interaction screening: 22 years of FAERS}
...
interactions each recovers. \textbf{We show that comparison is uninformative whenever
...
\textbf{Key result.} That the multiplicative assumption implies the additive one is
...
(RR\_A − 1)(RR\_B − 1). The unconditional form fails for 163 of
16,138 generated negative controls and for none of the
6,163 with both marginals above 1. The condition is met by
\textbf{98.3\%} of drug-dominant pairs against
\textbf{16.9\%} of diffuse ones, so in the regime
...
event was defined by \textbf{10 hand-curated MedDRA Preferred Terms in 3 concepts}
(the \texttt{core} tier of a 23-term, 10-concept curation; the remaining 13 terms form
the \texttt{broad} sensitivity tier),
...
\textbf{Contributions.} This paper provides six things a DDI screen can use
directly.
...
\tightlist
\item
  \textbf{The condition under which the choice of null stops mattering, and how
  ...
  conditions. It is not unconditional. With \emph{a} = RR\_A and \emph{b} = RR\_B the two
  expectations are proportional to \emph{ab} and \emph{a + b − 1}, and
  \emph{ab} − (\emph{a} + \emph{b} − 1) = (\emph{a} − 1)(\emph{b} − 1), so the implication requires both
  marginals on the same side of unity. We give the exact condition --- \textbf{both
  drugs elevated} --- and show it is not academic: among
  16,138 generated negative controls the unconditional form fails for
  \textbf{163} pairs, and for \textbf{none} of the
  6,163 pairs where both marginals exceed 1. The condition is
  met by \textbf{98.3\%} of drug-dominant pairs
  and \textbf{16.9\%} of diffuse ones, so in the
  ...
  examined (§4.3).
\item
  \textbf{The first calibrated error rates for both nulls in the regime where DDI
  methods are validated}, on a purpose-built pool of 2,345 strongly-associated
  non-interacting pairs: Ω₀₂₅ \textgreater{} 0 fires on \textbf{2.2\% (95\% CI 1.24--3.34\%)} and
  Ω\_add,₀₂₅ \textgreater{} 0 on \textbf{9.3\% (7.29--11.32\%)} against a nominal 2.5\% (§4.4).
\item
  \textbf{A construction for regime-matched negative controls}, which the standard
  frequency-matched generator cannot produce, and which is reusable for any
  endpoint (§4.4).
\item
  \textbf{A one-line diagnostic for circular screen evaluation} --- recompute
  enrichment over pairs containing no positive-control drug. Here it moves
  apparent enrichment from 2.02× to 1.12× (§4.8).
\item
  \textbf{An operating characteristic for DDI screening in this regime}, sweeping
  ...
  16 known interactions costs about \textbf{7.5\%}
...
\item
  \textbf{A polypharmacy cap that improves sensitivity and false-positive rate
  simultaneously}, from a quantification of pair-space leverage: 0.09\% of
  cases contribute 34.7\% of all drug pairs (§4.8).
\end{enumerate}

The screen itself returned no \emph{novel} interaction, and we report that as a
...
type: \textbf{0 orphans across all 303,663,833 child-table rows} --- every \texttt{DRUG},
\texttt{REAC}, \texttt{INDI}, \texttt{THER}, \texttt{OUTC} and \texttt{RPSR} row resolves to a \texttt{DEMO} row in its
own quarter. \texttt{DEMO} itself is the parent of that relation and so cannot be
...
\textbf{328,476,258 rows} in total.
...
\textbf{Two counts of the withdrawn cases, and why they differ.} FDA publishes
229,233 withdrawn case identifiers. \textbf{104,186} of them occur in \texttt{DEMO} and are
removed at stage 4; \textbf{98,102} occur in FAERS-era \texttt{DEMO} specifically. The
6,084-case difference is identifiers that appear \emph{only} under LAERS numbering ---
withdrawn FAERS \texttt{caseid} values that also occur as LAERS \texttt{case} numbers. That is
...
\texttt{results/tables/parse\_validation.csv}; the removal operates on the union.
...
\textbf{Every primary result uses the \texttt{core} tier: 10 PTs in 3 concepts} --- muscle
...
7 concepts form the \texttt{broad} tier, which adds general myotoxicity and the
...
interchangeable: \texttt{core} admits \textbf{42,058} event cases before the polypharmacy
exclusion and \texttt{broad} admits \textbf{339,063}, an eight-fold difference, so a reader
applying the wrong tier will not reproduce any count in this paper.

Two renamings occur within the concept area, both clean instantaneous switches: \texttt{BLOOD\ CREATINE\ PHOSPHOKINASE\ INCREASED} → \texttt{CREATINE\ KINASE\ INCREASED} at 2026Q2 (part of a vocabulary-wide
...
\section{4.2 When the two nulls stop being different tests}\label{when-the-two-nulls-stop-being-different-tests}

The comparison in §4.1 has a structural explanation, and it is checkable before
any outcome data is used.

\textbf{This is a refinement of a known relationship, not a new one.} Jung and Jung
...
they differ \textbf{only} through the expectation. The gamma-Poisson posterior
...
\textbf{When are they ordered?} Writing \emph{a} = RR\_A and \emph{b} = RR\_B, the multiplicative
expectation is proportional to \emph{ab} and the additive one to \emph{a + b − 1}, so
...
unconditional form fails for \textbf{163} pairs and for \textbf{none} of
the \textbf{6,163} where both marginals exceed 1. (The remaining
failures are pairs where both drugs are \emph{protective}; there the floor at
max(p\_A, p\_B) in E\_add binds and the proportionality above no longer governs.)

Where the expectations are ordered that way, the multiplicative null cannot
signal on a pair the additive null misses. The two are then \textbf{not competing
...
\begin{longtable}[]{@{}lrr@{}}
\toprule\noalign{}
pairs & n & E\_mult ≥ E\_add \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
expected joint rate \textgreater{} 5\% (drug-dominant) & 4,314 & \textbf{98.3\%} \\
expected joint rate ≤ 0.5\% (diffuse) & 3,214 & \textbf{16.9\%} \\
all screened pairs & 17,375 & 76.0\% \\
\end{longtable}
}

The implication holds without exception in the data: of the
\textbf{13,208} pairs whose expectations are ordered, \textbf{not one}
signals under Ω while failing under Ω\_add. All
\textbf{64} pairs that do so --- anywhere in the
...
\textbf{This is the diagnostic.} Both expectations are functions of the marginals and
the co-report count alone --- no outcome comparison, no control set, no
calibration. A screen can therefore determine \emph{before looking at any result}
...
\section{4.3 The estimand switch does not inflate sensitivity}\label{the-estimand-switch-does-not-inflate-sensitivity}
...
\section{4.4 Tier A and Tier B}\label{tier-a-and-tier-b}
...
\section{4.5 The operating characteristic, and what recovery costs}\label{the-operating-characteristic-and-what-recovery-costs}
...
needs to know what recovering \emph{k} known interactions costs in false positives
...
\bottomrule\noalign{}
\endlastfoot
1.0\% & +2.14 & \textbf{1/16} & +0.60 & 1/16 \\
2.5\% & +1.32 & \textbf{6/16} & -0.12 & 6/16 \\
5.0\% & +0.70 & \textbf{10/16} & -0.74 & 8/16 \\
7.5\% & +0.28 & \textbf{12/16} & -1.05 & 11/16 \\
10.0\% & -0.10 & \textbf{12/16} & -1.34 & 11/16 \\
15.0\% & -0.62 & \textbf{14/16} & -1.82 & 12/16 \\
20.0\% & -1.10 & \textbf{15/16} & -2.27 & 14/16 \\
30.0\% & -1.96 & \textbf{15/16} & -3.10 & 16/16 \\
\end{longtable}
}

Three things follow, and the first is the one a practitioner needs.

\textbf{Recovering 12 of the 16 known interactions costs about 7.5\% false positives
...
The conventional Ω\_add,₀₂₅ \textgreater{} 0 cut reaches the same 12 at 9.3\% (§4.4); the
saving from calibrating is about two points of false-positive rate, not an
order of magnitude.

\textbf{The additive null's advantage is real but small and not uniform.} It runs
from -1 to 2 pairs
...
\textbf{A strength-varying threshold was tried and is not recommended.} Calibrating
the threshold as a function of log₂(RR\_A × RR\_B) rather than holding it fixed
...
\section{4.6 Polypharmacy leverage (Figure 5)}\label{polypharmacy-leverage-figure-5}
...
\section{4.7 A third estimand, and why it does not settle the question}\label{a-third-estimand-and-why-it-does-not-settle-the-question}
...
produces more events than drug A produces \textbf{with its other partners}, and
...
Evaluated against both published nulls on \textbf{identical pairs} --- the
...
\bottomrule\noalign{}
\endlastfoot
2.5\% & \textbf{3/15} & 4/15 & 6/15 \\
5.0\% & \textbf{9/15} & 10/15 & 8/15 \\
7.5\% & \textbf{12/15} & 12/15 & 11/15 \\
10.0\% & \textbf{12/15} & 12/15 & 11/15 \\
15.0\% & \textbf{15/15} & 14/15 & 12/15 \\
20.0\% & \textbf{15/15} & 14/15 & 13/15 \\
\end{longtable}
}

\textbf{No estimand dominates.} The within-victim anchor is the only one to recover
...
using no global expectation at all. \textbf{The estimand is not where the leverage
...
\section{4.8 The screen shows no demonstrable enrichment for genuine interactions (Figures 3--4)}\label{the-screen-shows-no-demonstrable-enrichment-for-genuine-interactions-figures-34}
...
\section{4.9 Temporal stability: composition, not count (Figure 4)}\label{temporal-stability-composition-not-count-figure-4}
...
excluding unity. \textbf{That figure does not withstand the two corrections §4.8
...
\section{4.10 Confounding explains the unsupported era-stable pairs, but not all of them}\label{confounding-explains-the-unsupported-era-stable-pairs-but-not-all-of-them}
...
\texttt{label\_documented} (§4.8); it is also absent from the authors' own implicated-drug
list. The screen ranked a genuine contraindicated interaction first and both
evaluation references were blind to it.

\textbf{Ranking the screen, and separating signal from proxy.} The same third-drug
...
signals under Ω too
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
event cases with a third implicated drug
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
...
implicated drug on \textbf{64.7\%} and \textbf{100\%} of their event cases: these are the
``cardiovascular patient taking a statin'' pattern already described, and the
pair is not the cause. The top three sit at \textbf{8.4\%, 16.7\% and 19.5\%} --- they
carry their own signal.

Those top three are also \textbf{absent from every reference available to us}, and
...
\textbf{Abiraterone + rosuvastatin is the one entry that clears both nulls.} Its
Ω₀₂₅ of \textbf{+1.82} is remarkable in context: §4.4 shows the multiplicative null
...
this as a ranked observation and \textbf{make no mechanistic claim}: the cached FDA
...
event rate \emph{conditional} on co-reporting, which is what is elevated here. It is
...
\section{4.11 Conclusions do not depend on the unverified shrinkage constant (Figure 6)}\label{conclusions-do-not-depend-on-the-unverified-shrinkage-constant-figure-6}
...
\section{4.12 Sensitivity analyses}\label{sensitivity-analyses}
...
held-out control comparison (§4.13). Four of those \textbf{contradicted} claims in
...
adopted in §4.6 because it improved control recovery \emph{and} the false-positive
...
\textbf{Inpatient status, using the reported outcome code.} §4.10 explains why the
...
consistent with the known epidemiology of rhabdomyolysis. \textbf{Both
cluster-bootstrap intervals overlap unity}, and so does the crude male one.
The crude female interval does \emph{not} --- 2.13 (1.20--3.78) --- and it is the only
...
\section{4.13 Recovery on controls the authors did not choose}\label{recovery-on-controls-the-authors-did-not-choose}
...
\section{4.14 Does the failure generalise beyond this event?}\label{does-the-failure-generalise-beyond-this-event}
...
\item
  \textbf{Era bin boundaries are a researcher degree of freedom.} §4.12 shows the
...
\item
  ...
  FAERS-specific.
\item
  \textbf{Demographic subgroups are underpowered.} §4.12 reports sex-stratified
...
\item
  ...
  independent sources state α = 0.5 and the Ω₀₂₅ \textgreater{} 0 threshold (§2), and §4.11
...
\textbf{A screen can determine in advance whether its choice of null is a real
choice.} Both statistics are the same shrunk log-ratio and differ only through
the expectation, so wherever E\_mult ≥ E\_add the multiplicative signal set lies
...
\textbf{The thresholds those comparisons should use are supplied.} On a purpose-built
pool of 2,345 strongly-associated non-interacting pairs --- a construction the
standard frequency-matched generator cannot produce --- Ω₀₂₅ \textgreater{} 0 fires on 2.2\%
(95\% CI 1.24--3.34\%) and Ω\_add,₀₂₅ \textgreater{} 0 on 9.3\% (7.29--11.32\%) against a nominal
...
\item
  Jung Y, Jung H. A simulation-based comparison of drug--drug interaction
  signal detection methods. \emph{PLOS ONE.} 2024;19(4):e0300268.
...
The in-regime negative pool (\texttt{in\_regime\_pool.csv}, 19,826 pairs with the 2,345 in-regime ones flagged) and the per-drug marginal relative risks behind Table 2 (\texttt{rr\_a}/\texttt{rr\_b} in \texttt{tier\_a\_results.csv}) both ship, so the calibration rates and the mechanistic correlations can be recomputed without rebuilding the database.
