...
\emph{observed} joint event rate is flat in marginal strength (r = +0.12, p = 0.67).
...
0.039--0.191\%), implying \textbf{16.1 era-stable pairs by chance (95\% CI 6.8--33.2)
...
\section{3.7 Statistical measures}\label{statistical-measures}

The eight cells of the 2×2×2 table are recoverable from the triple count, six
marginals and the total.

\textbf{Ω (multiplicative null).} Fitted exactly by iterative proportional fitting
rather than the published closed-form approximation, which we measured at up to
237\% error once pairwise log-odds reach 2 --- enough to produce Ω \textless{} −1 on tables
with no synergy.

\textbf{Ω\_add (additive null), primary.} Expected joint risk P(Z\textbar A) + P(Z\textbar B) − P(Z),
floored at max(P(Z\textbar A), P(Z\textbar B)). Identical gamma-posterior shrinkage, so the two
...
\textbf{The observed joint event rate does not rise with marginal strength at all.}
...
Under the additive null, \textbf{12/16 controls signal (12/14 powered)}. Six of seven
simvastatin pairs and three of three colchicine pairs recover; both lovastatin
pairs have n\_ab of 19 and 1 and are unmeasurable.
...
\textbf{Against a nominal 2.5\%, Ω is about twice as conservative as advertised in this
regime and Ω\_add about four times too permissive.}
...
\textbf{1,712 of 17,375 pairs (9.8\%)} are undocumentable by construction and fall
...
proxy which could not have detected one. \textbf{Both strata reproduce the result}:
...
\emph{calibration} finding --- Ω again far more conservative than its nominal 2.5\%, the
...
\textbf{flat} in marginal strength (r = +0.12), while both nulls predict them to rise
...
\item
  ...
  itself, warns by class as well as by name, and is \textbf{absent entirely for 17.2\%
...
robust methodological contributions are the demonstration that \textbf{both}
...
\item
  ...
  \emph{Sci Data.} 2016. PMID 27193236. Data: doi:10.5061/dryad.8q0s4
\item
...
\item
  ...
  FAERS. \emph{Drug Saf.} 2024;47:271--284.
\item
...
\item
  ...
  effects and interactions. \emph{Sci Transl Med.} 2012. PMID 22422992.
...
\textbf{Secondary corroboration for the Ω definition} (see §2): a peer-reviewed
review of DDI statistical methodologies (\emph{Front Pharmacol.} 2019;10:1319) and
