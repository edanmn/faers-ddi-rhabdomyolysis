...
consistent with the known epidemiology of rhabdomyolysis. **Both
cluster-bootstrap intervals overlap unity**, and so does the crude male one.
The crude female interval does *not* — 2.13 (1.20–3.78) — and it is the only
non-control enrichment anywhere in this paper whose interval excludes unity. We
report it rather than only its clustered counterpart, because it is a clean
instance of the anticonservatism §3.8 describes: 15,471 pairs built from a few
hundred drugs are not independent trials, and resampling drugs moves the
