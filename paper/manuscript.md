...
0.66–0.89). The reference is also structurally blind to 9.8% of screened
pairs — 17.2% of ingredients have no FDA label at all, including cerivastatin,
the fibrates and fusidic acid — and restricting to pairs whose two labels both
...
0.039–0.191%), implying **16.1 era-stable pairs by chance (95% CI 6.8–33.2)
...
**Ω (multiplicative null).** Fitted exactly by iterative proportional fitting
rather than the published closed-form approximation, which we measured at up to
237% error once pairwise log-odds reach 2 — enough to produce Ω < −1 on tables
with no synergy.

**Ω_add (additive null), primary.** Expected joint risk P(Z|A) + P(Z|B) − P(Z),
floored at max(P(Z|A), P(Z|B)). Identical gamma-posterior shrinkage, so the two
differ only in the null.
...
(§3.6): 6,106 pairs in which one drug's label names the other. It captures
...
**1,712 of 17,375 pairs (9.8%)** are undocumentable by construction and fall
...
Note the direction of this bias. §3.6 states that under-sensitivity biases
...
- **The independent reference is FDA labelling, not a curated DDI database, and
  ...
  itself, warns by class as well as by name, and is **absent entirely for 17.2%
  of screened ingredients — 9.8% of pairs**, among them cerivastatin, the
  fibrates and fusidic acid. §4.5 reports the analysis restricted to
