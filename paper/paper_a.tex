...
What survives is mechanistic: the observed joint event rate rises far more shallowly with marginal strength (\emph{r} = +0.12, 95\% CI −0.40 to +0.58) than either null predicts (\emph{r} ≈ +0.94), so joint risk saturates and both models miss it in the same direction, differing in the level of their expectation rather than its gradient. The recommendation follows: \textbf{compute the marginal relative risks first, then calibrate against negative controls from the same regime} --- neither null is usable at its nominal cut, one for want of power and the other for want of calibration. We also report a design constraint positive controls cannot reveal: 19,005 cases (0.09\%) contribute 34.7\% of all drug pairs at a 4× enriched event rate.
\end{abstract}
\vspace{2ex}
\end{@twocolumnfalse}]
...
\emph{Caveat.} The matched-rate calibration rests on the 166 in-regime pairs of the generated pool, so the 5\%/10\%/20\% points sit on roughly 8/17/33 of them and are noisy (Section 7).
...
What replicates on torsade is therefore the \emph{asymmetry}, not the recovery finding: Ω again runs close to its nominal 2.5\% (2.0\%, on 152 in-regime negatives) while the additive null runs far above it (42.8\%). On neither event is Ω's error rate the problem. What does \textbf{not} replicate is that the additive null recovers more at a common error rate. On the primary event it retains a one-to-two-pair advantage; here it retains none.
...
Neither disproportionality null is usable at its conventional operating point when the drugs under study are the dominant reported causes of the outcome, but they fail in different currencies. Against a purpose-built pool of 2,345 strongly-associated non-interacting pairs, \(\Omega_{025}\) \textgreater{} 0 fires on 2.2\% and \(\Omega_{\mathrm{add},025}\) \textgreater{} 0 on 9.3\% against a nominal 2.5\%; on a second event, 2.0\% and 42.8\%. \textbf{Only the additive null is miscalibrated on error rate.} Ω holds its advertised rate on both events and is disqualified by power instead, buying that rate by almost never firing. Conventionally generated negative controls reveal neither failure: they exclude the pair configuration every positive control has and sit two orders of magnitude away in marginal strength.
